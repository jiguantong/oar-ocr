...
/// CoreML hardware selection policy.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum OrtCoreMLComputeUnits {
    /// Let CoreML choose between CPU, GPU, and Neural Engine.
    All,
    /// Restrict CoreML to CPU execution.
    CpuOnly,
    /// Allow CPU and GPU execution.
    CpuAndGpu,
    /// Allow CPU and Neural Engine execution.
    CpuAndNeuralEngine,
}

/// CoreML model representation generated by ONNX Runtime.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum OrtCoreMLModelFormat {
    /// Core ML 5+ representation with broader operator support.
    MlProgram,
    /// Legacy Core ML neural-network representation.
    NeuralNetwork,
}
...
pub enum OrtExecutionProvider {
    ...
    CoreML {
        /// Hardware devices CoreML may use.
        compute_units: Option<OrtCoreMLComputeUnits>,
        /// CoreML representation generated from the ONNX graph.
        model_format: Option<OrtCoreMLModelFormat>,
        /// Directory used to persist compiled CoreML models across session loads.
...
mod tests {
    ...
    #[test]
    fn coreml_options_roundtrip_through_json() {
        let provider = OrtExecutionProvider::CoreML {
            ...
            model_cache_dir: Some("/tmp/oar-coreml".to_string()),
            subgraphs: Some(false),
        };

        let json = serde_json::to_string(&provider).expect("serialize CoreML provider");
        let decoded: OrtExecutionProvider =
            serde_json::from_str(&json).expect("deserialize CoreML provider");

        assert_eq!(decoded, provider);
        assert!(json.contains("ml_program"));
    }
}
